...
class CardinalLSTMTransformer(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X: pd.DataFrame, y=None, *args, **kwargs):
        self.model = AttentionModel(self.model_name,
                                    ...
                                    self.hidden_dim, self.layer_num, self.learning_rate, self.dropout, self.embedding_dim)
...
if __name__ == '__main__':
    df = pd.DataFrame({'before': '123 в 1902 году 56 пехотинцев'.split(),
                             'class': ['CARDINAL', 'PLAIN', 'CARDINAL', 'PLAIN', 'CARDINAL', 'PLAIN']})
    ...
    ct = CardinalLSTMTransformer(modelpath='cardinal_epoch_8_0.8895194854332198_0_64_2_0.0_0.0001.hdf5')
