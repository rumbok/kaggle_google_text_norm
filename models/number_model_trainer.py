...
y_data = case_pipeline.fit_transform(df['after'])['number'].cat.codes
...
dtrain.save_binary('number.matrix.train.train')
dtest.save_binary('number.matrix.train.test')
del dtrain
del dtest
gc.collect()


dtrain = xgb.DMatrix('number.matrix.train.train#number.dtrain.cache')
dtest = xgb.DMatrix('number.matrix.train.test#number.dtest.cache')
...
model.save_model(
    f'number.model.train_{len(dtrain.get_label())}_{1.0-accuracy:0.5f}_{param["learning_rate"]}_{param["num_boost_round"]}_{param["max_depth"]}')
