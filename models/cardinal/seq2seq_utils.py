...
def load_data(max_len, input_vocab_size, output_vocab_size):
    CARDINAL_REGEXP = '\d'
    ...
    df['before'] = df['prev_prev'].map(str) + ' ' + df['prev'].map(str) + ' ' + df['before'].map(
        lambda s: ' '.join(list(s))) + ' ' + df['next'].map(str) + ' ' + df['next_next'].map(str)
    ...
    X = [text_to_word_sequence(w) for w in X_data]
    y = [text_to_word_sequence(w) for w in y_data]
